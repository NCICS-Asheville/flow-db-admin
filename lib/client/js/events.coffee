...
Template.AdminDashboardUsersEdit.events
	'click .btn-add-role': (e,t) ->
		...
		Meteor.call 'adminAddUserToRole', $(e.target).attr('user'), $(e.target).attr('role'), $(e.target).attr('group')
	'click .btn-remove-role': (e,t) ->
		console.log 'removing user from role'
		Meteor.call 'adminRemoveUserToRole', $(e.target).attr('user'), $(e.target).attr('role'), $(e.target).attr('group')
